...
export function LevelsPage() {
  ...
  const { channels, roles, settings, isLoading: contextLoading } = useDashboardInit(serverId);
  ...
  useEffect(() => {
    if (!contextLoading && settings) {
      if (settings.levels) setLevelData({ ...defaultLevelData, ...settings.levels });
      setIsLoading(false);
    }
  ...
  const updateRoleReward = (index: number, field: 'level' | 'roleId', value: any) => {
    setLevelData(prev => ({
      ...prev,
  ...
  return (
    <div className="max-w-[1000px] mx-auto">
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white mb-1">Seviye Sistemi</h1>
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              ...
              onChange={(e) => setLevelData(prev => ({ ...prev, enabled: e.target.checked }))}
      ...
      {/* Stats Cards */}
      <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 ${!levelData.enabled ? 'opacity-50 pointer-events-none' : ''}`}>
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <input
            ...
            onChange={(e) => setLevelData(prev => ({ ...prev, xpPerMessage: parseInt(e.target.value) || 0 }))}
            min={1}
            max={100}
        ...
        >
          ...
          <input
            ...
            onChange={(e) => setLevelData(prev => ({ ...prev, cooldown: parseInt(e.target.value) || 0 }))}
            min={0}
            max={3600}
            className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-2xl font-bold text-white focus:outline-none focus:border-accent/50 text-center"
          />
        ...
        >
          ...
          <div className="text-2xl font-bold text-white text-center py-3">
            ~{levelData.cooldown > 0 ? Math.round((60 / levelData.cooldown) * levelData.xpPerMessage) : levelData.xpPerMessage}
          </div>
        </motion.div>
      </div>
      ...
      <motion.div
        ...
        className={`mb-6 p-6 bg-surface rounded-xl border border-white/10 ${!levelData.enabled ? 'opacity-50 pointer-events-none' : ''}`}
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[5, 10, 25, 50].map(level => (
            <div key={level} className="p-4 bg-white/5 rounded-xl text-center">
              ...
              <p className="text-white font-semibold">{Math.round(calculateXpForLevel(level)).toLocaleString()} XP</p>
              <p className="text-xs text-slate-500 mt-1">
                ~{Math.round(calculateXpForLevel(level) / levelData.xpPerMessage)} mesaj
              </p>
            </div>
          ))}
        </div>
      ...
      <motion.div
        ...
        className={`mb-6 p-6 bg-surface rounded-xl border border-white/10 ${!levelData.enabled ? 'opacity-50 pointer-events-none' : ''}`}
      >
        <h2 className="text-lg font-semibold text-white mb-4">Seviye Atlama Bildirimi</h2>
        <div className="space-y-4">
          <div>
            ...
            <select
              ...
              onChange={(e) => setLevelData(prev => ({ ...prev, levelUpChannelId: e.target.value }))}
          ...
          <div>
            <label className="text-sm font-medium text-slate-300 mb-2 block">Mesaj</label>
            <textarea
              value={levelData.levelUpMessage}
              onChange={(e) => setLevelData(prev => ({ ...prev, levelUpMessage: e.target.value }))}
              rows={3}
            ...
            {levelData.roles.map((roleReward, index) => (
              <div key={index} className="flex items-center gap-3 p-3 bg-white/5 rounded-lg">
                <div className="flex-1 grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-slate-400 mb-1 block">Seviye</label>
                    <input
                      type="number"
      ...
      {/* Save Button */}
      <div className={`flex justify-end ${!levelData.enabled ? 'opacity-50 pointer-events-none' : ''}`}>
