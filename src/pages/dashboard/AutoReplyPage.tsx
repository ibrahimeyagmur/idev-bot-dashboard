...
export function AutoReplyPage() {
  ...
  const { settings: cachedSettings, isLoading: contextLoading } = useDashboardInit(serverId);
  const [settings, setSettings] = useState<AutoReplySettings>({ enabled: false, rules: [] });
  ...
  const saveSettings = async () => {
    setSaving(true);
    try {
      const res = await fetch(`http://localhost:3001/api/server/${serverId}/autoreply`, {
        method: 'PUT',
  ...
  const updateRule = () => {
    if (!editingRule || !editingRule.keyword.trim() || !editingRule.reply.trim()) return;
  ...
  const filteredRules = settings.rules.filter(
    r => r.keyword.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-white">Otomatik Cevap Sistemi</h2>
              <p className="text-sm text-slate-400">Sistemi aktif veya pasif yapın</p>
            </div>
          </div>
      ...
      >
        ...
        <div className={`${!settings.enabled ? 'opacity-50 pointer-events-none' : ''}`}>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-semibold text-white">Kurallar ({settings.rules.length})</h2>
            <Button size="sm" onClick={() => setShowModal(true)} disabled={!settings.enabled}>
        ...
        {filteredRules.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-slate-500">Yeni bir kural ekleyerek başlayın</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        <span className={`text-sm font-medium ${rule.enabled ? 'text-white' : 'text-slate-400'}`}>
                          {rule.keyword}
                        </span>
                      ...
                      <p className="text-sm text-slate-400 truncate">{rule.reply}</p>
                    </div>
                  </div>
        ...
        <div className="mt-6 pt-6 border-t border-white/10">
          <Button onClick={saveSettings} disabled={saving || !settings.enabled}>
            {saving ? (
      ...
      <AnimatePresence>
        {showModal && (
          ...
          >
            ...
            >
              <h3 className="text-lg font-semibold text-white mb-4">Yeni Kural Ekle</h3>

              <div className="space-y-4">
                <div>
                  <label className="text-sm font-medium text-slate-300 mb-2 block">Anahtar Kelime</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-sm font-medium text-slate-300 mb-2 block">Eşleşme Türü</label>
                  <select
                    value={newRule.match}
                    onChange={(e) => setNewRule(prev => ({ ...prev, match: e.target.value as 'contains' | 'exact' }))}
                ...
                <div>
                  <label className="text-sm font-medium text-slate-300 mb-2 block">Yanıt</label>
                  <textarea
                    value={newRule.reply}
                    onChange={(e) => setNewRule(prev => ({ ...prev, reply: e.target.value }))}
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <Button onClick={addRule} disabled={!newRule.keyword.trim() || !newRule.reply.trim()}>
                  <Plus className="w-4 h-4 mr-1" />
                  Ekle
                </Button>
              </div>
        ...
        {editingRule && (
          ...
          >
            <motion.div
              ...
              className="bg-background border border-white/10 rounded-xl w-full max-w-md p-6"
            >
              <h3 className="text-lg font-semibold text-white mb-4">Kuralı Düzenle</h3>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setEditingRule(prev => prev ? { ...prev, match: e.target.value as 'contains' | 'exact' } : null)}
                    className="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white focus:outline-none focus:border-accent/50"
                  >
                    <option value="contains">İçerir</option>
                    <option value="exact">Tam Eşleşme</option>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setEditingRule(prev => prev ? { ...prev, reply: e.target.value } : null)}
                    rows={3}
