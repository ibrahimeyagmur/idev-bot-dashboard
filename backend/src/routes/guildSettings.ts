...
router.put("/:serverId/welcome", (req, res) => {
  ...
  console.log(
    `[Welcome] Saved settings for ${serverId}:`,
    JSON.stringify(welcomeSettings)
  );
...
router.put("/:serverId/leave", (req, res) => {
  ...
  console.log(
    `[Leave] Saved settings for ${serverId}:`,
    JSON.stringify(leaveSettings)
  );
...
router.post("/:serverId/embeds", (req, res) => {
  const { serverId } = req.params;
  const { title, description, color, thumbnail, image, footer, fields } =
    req.body;
