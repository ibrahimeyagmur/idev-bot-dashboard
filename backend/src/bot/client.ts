...
client.on(Events.GuildMemberAdd, async (member) => {
  console.log(`[Welcome] Üye katıldı: ${member.user.tag} -> ${member.guild.name}`);
  ...
  const channel = member.guild.channels.cache.get(settings.channelId) as TextChannel;
  if (!channel) return;
  ...
  try {
    if (settings.messageType === 'embed') {
      ...
      const description = (embedData.description || settings.message || 'Hoş geldin {user}!')
        .replace(/{user}/g, `<@${member.id}>`)
        .replace(/{server}/g, member.guild.name)
        .replace(/{count}/g, member.guild.memberCount.toString());
      
      const embed = new EmbedBuilder()
        .setColor(parseInt((embedData.color || '#57F287').replace('#', ''), 16));
...
client.on(Events.GuildMemberRemove, async (member) => {
  console.log(`[Leave] Üye ayrıldı: ${member.user?.tag} -> ${member.guild.name}`);
  ...
  try {
    if (settings.messageType === 'embed') {
      const embedData = settings.embed || {};
      const description = (embedData.description || settings.message || '{user} aramızdan ayrıldı.')
        .replace(/{user}/g, member.user?.username || 'Bilinmeyen')
        .replace(/{server}/g, member.guild.name)
        .replace(/{count}/g, member.guild.memberCount.toString());
      
      const embed = new EmbedBuilder()
        .setColor(parseInt((embedData.color || '#ED4245').replace('#', ''), 16));
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (userData.level > oldLevel) {
    const levelUpMessage = (settings.levelUpMessage || '🎉 {user} seviye {level} oldu!')
    ...
    if (settings.levelUpChannelId) {
      channel = message.guild.channels.cache.get(settings.levelUpChannelId) as TextChannel;
    }
    if (!channel) {
...
const profanityList = [
  'amk', 'aq', 'amına', 'amina', 'amını', 'amini', 'amcık', 'amcik',
  'orospu', 'oç', 'oc', 'piç', 'pic', 'sik', 'sikik', 'sikim', 'sikeyim',
  'yarrak', 'yarak', 'taşak', 'tasak', 'göt', 'got', 'meme',
  'ananı', 'anani', 'ananın', 'ananin', 'bacını', 'bacini',
  'pezevenk', 'kahpe', 'ibne', 'top', 'gavat',
  'siktir', 'sg', 'amq', 'mk', 'awk',
];
...
client.on(Events.MessageCreate, async (message) => {
  ...
  const isIgnored = (settings: any) => {
    ...
    if (settings.ignoredRoleIds?.some((roleId: string) => message.member?.roles.cache.has(roleId))) return true;
    return false;
  };
  ...
  if (automod.profanity?.enabled && !isIgnored(automod.profanity)) {
    ...
    const hasProfanity = profanityList.some(word => lowerContent.includes(word.toLowerCase()));
